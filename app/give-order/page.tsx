...
const inputClass = "mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm text-slate-900 outline-none transition focus:border-[#2014FF] focus:bg-white focus:ring-2 focus:ring-blue-100 min-h-[48px]";
...
export default function GiveOrderPage() {
  ...
  if (result) return <main className="min-h-screen bg-slate-50 px-4 py-16"><div className="mx-auto max-w-lg rounded-3xl bg-white p-6 sm:p-8 text-center shadow-xl"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-2xl text-emerald-700">✓</div><h1 className="mt-5 text-xl sm:text-2xl font-extrabold text-slate-900">Order submitted</h1><p className="mt-2 text-sm sm:text-base text-slate-600">Thank you. Prime Capital has received your trade order and will review it shortly.</p><button onClick={() => setResult(false)} className="mt-6 w-full sm:w-auto rounded-xl bg-[#01016F] px-5 py-3 text-sm font-bold text-white min-h-[48px]">Give another order</button></div></main>;

  return <main className="min-h-screen bg-[#f5f7fc] pb-20 text-slate-900">
    <section className="relative isolate overflow-hidden bg-[#01016F] px-4 py-14 text-white sm:py-28" style={{ backgroundImage: "linear-gradient(90deg, rgba(1,1,111,.96), rgba(12,10,166,.88)), url('/herobg.png')", backgroundPosition: "center", backgroundSize: "cover" }}><div className="absolute -right-24 top-0 h-80 w-80 rounded-full bg-blue-300/10 blur-3xl" /><div className="relative mx-auto max-w-5xl"><p className="text-xs font-bold uppercase tracking-[0.25em] text-blue-200">Prime Capital S.C. · Brokerage Services</p><div className="mt-7 grid gap-8 md:grid-cols-[1.5fr_.75fr] md:items-end"><div><h1 className="max-w-3xl text-4xl font-extrabold tracking-tight sm:text-6xl">Place your trade order with clarity.</h1><p className="mt-6 max-w-2xl text-base leading-relaxed text-blue-100 sm:text-lg">Send a secure buy or sell instruction to our brokerage team. We will review the order and keep you informed of its status.</p></div><div className="border-l border-white/25 pl-5 text-sm leading-relaxed text-blue-100"><p className="font-bold uppercase tracking-wider text-white">Trading notice</p><p className="mt-2">Orders received after 9:00 local time are processed in the next trading session.</p></div></div></div></section>
